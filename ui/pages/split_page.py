...
class SplitPage(StagePageMixin, QWizardPage):
    def __init__(self, wizard) -> None:
        ...
        self.setSubTitle(
            "Random hold-out is fast; k-fold gives more stable estimates; spatial "
            "block CV is the honest choice for spatially autocorrelated data."
        )
    ...
    def _on_done(self, result) -> None:
        ...
        if plan is not None and fold_id is not None:
            ...
            self.summary_label.setText(
                ...
                f"{plan.n_blocks_x}×{plan.n_blocks_y} blocks, {len(folds)} fold(s)."
            )
            self.canvas.set_fold_colors(px, py, fold_id, crs=crs)
            self.canvas.set_block_polygons(
                wizard.session.stack.bounds, plan.block_size, plan.n_blocks_x, plan.fold_of_block, crs=crs,
            )
            show_spatial_block(
                wizard.iface, "Split", "fold_preview", px, py, fold_id,
                wizard.session.stack.bounds, plan.block_size, plan.n_blocks_x, plan.fold_of_block, crs=crs,
            )
        else:
